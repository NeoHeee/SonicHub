...
SonicHub 不替代 Songloft 或 Audiobookshelf 服务端，而是作为统一的客户端入口：

- 通过 Songloft API 登录、读取音乐内容并控制播放设备；
- 通过 Songloft 的 MIoT 插件发现和控制智能音箱；
- 连接 Audiobookshelf 书库，支持有声书、章节和播放进度；
- 支持音频直链播放，便于测试和接入其他音源；
- 使用统一的播放设备、播放状态和进度反馈，减少不同音源之间的操作差异。

## 当前版本

当前开发版本为 **v0.6.1**，本版本重点完成统一页面结构和“本机播放”能力：

- 统一首页：展示当前播放内容、音源入口和播放状态；
- 音乐：管理 Songloft 歌单、曲库层级和歌曲操作；
- 有声书：支持书库、继续收听、章节定位和进度回传；
- 多音源：集中展示 Songloft、Audiobookshelf、音频直链等入口；
- 设备：支持默认音箱、设备状态刷新、设备详情和投送反馈；
- 本机播放：音乐、有声书和音频直链均可在手机或蓝牙设备播放；
- 播放控制：支持播放、暂停、继续、停止和进度拖动；
- 交互反馈：统一加载、空状态、错误提示、重试和连接状态；
- 发布流程：使用固定 Android 签名，并通过 GitHub Actions 完成分析、测试和 Release APK 构建。
